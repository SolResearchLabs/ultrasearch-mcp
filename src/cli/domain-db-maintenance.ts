...
// restart) - NOT as an in-process timer: ultrasearch-mcp runs as several concurrent
...
export async function emitGauges(data: GaugeData): Promise<boolean> {
  ...
  // Attribute the metrics to ultrasearch-mcp via the env-based resource detector.
  if (!process.env.OTEL_SERVICE_NAME) {
    process.env.OTEL_SERVICE_NAME = "ultrasearch-mcp";
  }
  try {
    const [
      { OTLPMetricExporter },
    ...
    const meter = provider.getMeter("ultrasearch-mcp-maintenance");
