...
function warnIfCleartextLlmCredential(): void {
  ...
  if (LLM_API_KEY && LLM_BASE_URL.startsWith("http://")) {
    ...
    console.error(
      `[ultrasearch-mcp] LLM_API_KEY is set with a plain-http LLM_BASE_URL ("${LLM_BASE_URL}") - the bearer token will transmit in cleartext. Use an https:// LLM_BASE_URL, or confirm this is an internal/trusted network.`,
    );
  }
}
