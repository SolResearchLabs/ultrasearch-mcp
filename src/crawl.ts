...
async function fetchSitemapXml(url: string): Promise<string | null> {
  try {
    ...
    const res = await safeFetch(url, {
      headers: { "User-Agent": "ultrasearch-mcp" },
      signal: AbortSignal.timeout(10_000),
    });
    if (!res.ok) return null;
...
export async function bfsCrawl(
  ...
  while (queue.length > 0 && visited.size < maxPages) {
    ...
    const robots = await checkRobots(normalizedUrl, "ultrasearch-mcp");
...
export async function crawlSite(
  ...
  if (cached) {
    try {
      ...
      // crawl via the catch block. Auditor confirmed no action required. Audit: 2026-06-05/ultrasearch-mcp-crawl-2026-06.
