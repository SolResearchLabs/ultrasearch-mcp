...
export async function initEvents(): Promise<void> {
  ...
  try {
    ...
    if (process.env.NATS_CREDS) {
      ...
      // Forge's ultrasearch-mcp NATS user is bcrypt username/password (no .creds
    ...
    if (!warnedOnce) {
      console.error(
        `[ultrasearch-mcp] NATS connect failed (events disabled): ${err instanceof Error ? err.message : String(err)}`,
      );
      warnedOnce = true;
    }
    nc = null;
