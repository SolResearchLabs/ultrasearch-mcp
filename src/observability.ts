...
export async function initObservability(): Promise<void> {
  ...
  try {
    ...
    const serviceName = process.env.OTEL_SERVICE_NAME ?? "ultrasearch-mcp";
    ...
    tracer = otelApi.trace.getTracer("ultrasearch-mcp", VERSION);
    meter = otelApi.metrics.getMeter("ultrasearch-mcp", VERSION);
    ...
    console.error(
      `[ultrasearch-mcp] OTLP init failed: ${err instanceof Error ? err.message : String(err)}`,
    );
    tracer = null;
    meter = null;
    otelApi = null;
