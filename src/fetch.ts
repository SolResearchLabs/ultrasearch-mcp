...
async function runTier<T extends TierResult | null>(
  ...
  try {
    ...
    if (classified.disposition === "tier_skipped") {
      ...
      console.error(
        `[ultrasearch-mcp] fetch ${tier} skipped url=${url} reason=${classified.reason}`,
      );
      return null as T;
    }
...
export async function fetchPage(
  ...
  return withSpan("fetch", { "fetch.url": url }, async () => {
    ...
    if (isGithubUrl(url)) {
      ...
      const robots = await checkRobots(url, "ultrasearch-mcp");
      if (!robots.allowed) {
        console.error(
          `[ultrasearch-mcp] fetch skipped_robots url=${url} reason=${robots.reason ?? "disallowed"}`,
        );
      ...
      for (const { tier: slot, reason } of skipDecisions) {
        ...
        console.error(
          `[ultrasearch-mcp] fetch ${slot} skipped url=${url} reason=${reason}`,
        );
      }
      ...
      for (const tier of activeTiers) {
        ...
        if (fetched) {
          ...
          if (tier.slot !== "tier1") {
            console.error(
              `[ultrasearch-mcp] fetch ${tier.slot} hit url=${url}`,
            );
          }
          break;
        }
        console.error(`[ultrasearch-mcp] fetch ${tier.slot} miss url=${url}`);
      }

      if (!fetched && WAYBACK_ENABLED) {
        console.error(
          `[ultrasearch-mcp] fetch tier4_wayback attempt url=${url}`,
        );
        ...
        if (fetched) {
          ...
          console.error(`[ultrasearch-mcp] fetch tier4_wayback hit url=${url}`);
        } else {
          console.error(
            `[ultrasearch-mcp] fetch tier4_wayback miss url=${url}`,
          );
        }
      }
