...
export const CLOUDFLARE_BROWSER_API_TOKEN = configString(
  [
    "ULTRASEARCH_CLOUDFLARE_BROWSER_API_TOKEN",
    "CLOUDFLARE_BROWSER_API_TOKEN",
    "CLOUDFLARE_API_TOKEN",
  ],
  "providers.cloudflare.browserApiToken",
  "",
);
...
export const TRANSPORT = configString(
  ["ULTRASEARCH_TRANSPORT", "SEARXNG_MCP_TRANSPORT"],
  "transport.mode",
  "stdio",
);
export const HTTP_PORT = parseInt(
  configString(
    ["ULTRASEARCH_HTTP_PORT", "SEARXNG_MCP_PORT"],
    "transport.port",
    "3001",
  ),
  10,
);
export const HTTP_HOST = configString(
  ["ULTRASEARCH_HTTP_HOST", "SEARXNG_MCP_HOST"],
  "transport.host",
  "127.0.0.1",
);
...
export const RERANK_RECENCY_WEIGHT = (() => {
  ...
  if (Number.isNaN(v) || v < 0) {
    console.warn(
      `[ultrasearch-mcp] RERANK_RECENCY_WEIGHT="${process.env.RERANK_RECENCY_WEIGHT}" is invalid; recency weighting disabled.`,
    );
    return 0;
  }
  if (v > 1) {
    console.warn(
      `[ultrasearch-mcp] RERANK_RECENCY_WEIGHT=${v} exceeds 1.0; recency may dominate relevance scores.`,
    );
  }
  return v;
})();
