...
describe("log helpers", () => {
  ...
  it("logError/logWarn write to stderr with the [ultrasearch-mcp] prefix", () => {
    logError("boom");
    logWarn("careful");
    expect(errSpy).toHaveBeenNthCalledWith(1, "[ultrasearch-mcp] boom");
    expect(errSpy).toHaveBeenNthCalledWith(2, "[ultrasearch-mcp] careful");
  });
  ...
  it("logThrottled dedupes repeats for the same key within the interval", () => {
    ...
    expect(errSpy).toHaveBeenCalledWith("[ultrasearch-mcp] first");
  });
