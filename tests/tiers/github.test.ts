...
describe("isGithubUrl", () => {
  it("matches github.com", () => {
    expect(
      isGithubUrl("https://github.com/SolResearchLabs/ultrasearch-mcp"),
    ).toBe(true);
  });
  ...
  it("matches raw.githubusercontent.com", () => {
    expect(
      isGithubUrl(
        "https://raw.githubusercontent.com/SolResearchLabs/ultrasearch-mcp/main/README.md",
      ),
    ).toBe(true);
  });
  ...
  it("matches api.github.com", () => {
    expect(
      isGithubUrl(
        "https://api.github.com/repos/SolResearchLabs/ultrasearch-mcp",
      ),
    ).toBe(true);
  });
...
describe("githubFetch - raw.githubusercontent.com", () => {
  const RAW_URL =
    "https://raw.githubusercontent.com/SolResearchLabs/ultrasearch-mcp/main/src/index.ts";
...
describe("githubFetch - api.github.com", () => {
  const README_URL =
    "https://api.github.com/repos/SolResearchLabs/ultrasearch-mcp/readme";
  ...
  it("decodes base64 README-shaped responses", async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      jsonResponse({
        ...
        name: "README.md",
        html_url:
          "https://github.com/SolResearchLabs/ultrasearch-mcp/blob/main/README.md",
      }),
    );
  ...
  it("falls back to pretty-printed JSON for non-README endpoints", async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      jsonResponse({
        full_name: "SolResearchLabs/ultrasearch-mcp",
        stargazers_count: 3,
      }),
    );
    ...
    expect(result.title).toBe("/repos/SolResearchLabs/ultrasearch-mcp");
...
describe("githubFetch - github.com", () => {
  it("rewrites a blob URL to raw content and fetches it", async () => {
    ...
    const result = await githubFetch(
      "https://github.com/SolResearchLabs/ultrasearch-mcp/blob/main/scripts/hi.py",
    );
    expect(mockFetch).toHaveBeenCalledWith(
      "https://raw.githubusercontent.com/SolResearchLabs/ultrasearch-mcp/main/scripts/hi.py",
      expect.anything(),
    );
  ...
  it("fetches the repo README for a repo-root URL", async () => {
    const content = Buffer.from("# ultrasearch-mcp").toString("base64");
    mockFetch.mockResolvedValueOnce(
      jsonResponse({
        content,
        name: "README.md",
    ...
    const result = await githubFetch(
      "https://github.com/SolResearchLabs/ultrasearch-mcp",
    );
    expect(mockFetch).toHaveBeenCalledWith(
      "https://api.github.com/repos/SolResearchLabs/ultrasearch-mcp/readme",
      expect.anything(),
    );
    expect(result.text).toBe("# ultrasearch-mcp");
    expect(result.title).toBe("SolResearchLabs/ultrasearch-mcp - README.md");
  });
});
